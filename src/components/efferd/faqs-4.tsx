...
const categories = [
	...
	},
	{
		icon: (
			<IconPlaceholder
				className="size-4"
				hugeicons="FeatherIcon"
				lucide="FeatherIcon"
				phosphor="FeatherIcon"
		...
		<section className="container">
			...
			<div className="relative grid min-h-full grid-cols-1 py-12 md:grid-cols-3">
					...
					<div className="flex w-max flex-wrap items-start justify-start gap-2 md:flex-col md:justify-center">
						{categories.map((cat) => (
							<Button
								...
								className={activeCategory === cat.id ? undefined : "bg-muted"}
							>
								{cat.icon}
								{cat.label}
							</Button>
				...
				<div className="col-span-2 space-y-5 px-4 py-5">
					...
					<Accordion
						className="space-y-2"
						defaultValue={filtered[0] ? [filtered[0].id.toString()] : []}
						key={activeCategory}
					>
						{filtered.map((item) => (
							<AccordionItem
